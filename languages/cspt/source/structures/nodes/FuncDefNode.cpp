...
FuncDefNode::FuncDefNode(Token _name_tok, std::vector<Token> _arg_name_toks, std::shared_ptr<Node> _body_node, bool _should_auto_return)
...
std::string FuncDefNode::to_string(int depth)
{
  ...
  {
    res += std::string(2 * depth, ' ') + arg_name_toks[i].to_string() + (i < arg_name_toks.size() - 1 ? ",\n" : "");
  }
